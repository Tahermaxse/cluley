...
const Features: React.FC<FeaturesProps> = ({ title, subtitle, features }) => {
  return (
    <div
      id="features"
      className="mx-auto max-w-7xl pt-20 lg:pt-65 select-none lg:border-x border-zinc-200 overflow-hidden"
    >
      ...
      <div className="grid lg:grid-cols-2 mt-20 mb-16 lg:mb-0 pointer-events-none lg:border-y border-zinc-200 lg:divide-x divide-zinc-200">
        {features.slice(0, 2).map((feature, index) => (
          <div key={index} className="relative">
            ...
            <div className="hidden lg:block absolute bg-[#0055FE] w-1 h-8.5 bottom-28.5 -left-[1px]" />
          </div>
        ))}
      </div>
      {features.slice(2).map((feature, index) => (
        <div
          key={index + 2}
          className="relative pointer-events-none lg:border-b border-zinc-200"
        >
          <img src={feature.imageSrc} alt={feature.imageAlt || feature.title} />
          ...
          <div className="hidden lg:block absolute bg-[#0055FE] w-1 h-8.5 bottom-34 -left-[1px]" />
        </div>
      ))}
    </div>
  );
